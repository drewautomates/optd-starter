...
|---|---|---|
| 9 | [How I Trade 5 Lucid Prop Firm Accounts From Sierra Chart](https://youtu.be/Gmom23zFzAI) | The Sierra Chart → Lucid (Rithmic) **click-path checklist** ([`sierra/LUCID-RITHMIC-CHECKLIST.md`](sierra/LUCID-RITHMIC-CHECKLIST.md)): one lead account in Sierra Chart, four followers copied by Rithmic's Trade Copier, the per-view Trade Account rule, the pass condition, and why the copier is not atomic. Starts from the chartbook ([`sierra/chartbooks/OPTD.Cht`](sierra/chartbooks/OPTD.Cht)). Plumbing, not a strategy |
| 8 | [How to Trade TopstepX From Sierra Chart — Keep the Charts You Actually Use (Free Code)](https://youtu.be/PGZLzlDdw9I) | No new slice here: the Sierra Chart → TopstepX manual mirror ships in its own repo, [`optd-sierrachart-topstepx-mirror`](https://github.com/drewautomates/optd-sierrachart-topstepx-mirror). You click the order in Sierra Chart and it copies it to TopstepX through the ProjectX API. Plumbing, not a strategy |
| 7 | [Sierra Chart Automated Trading: How to Build a System That Doesn't Blow Up (Claude Code)](https://youtu.be/qiMaH6IiZ3Y) | The **order manager starter prompt** — seven requirements (arming switch that defaults to OFF, bracket on entry, one position at a time, size as an input, emergency flatten, mismatch handling, bar-close gate) as a paste-ready file ([`research/prompts/order_manager_starter.md`](research/prompts/order_manager_starter.md)). Drives the ORB signal study ([`sierra/studies/OPTD_Opening_Range.cpp`](sierra/studies/OPTD_Opening_Range.cpp)) in SIM. The prompt is the chassis; the method is in the video |
| 6 | [Sierra Chart Setup 2026: Which Package Should You Buy? (Don't Overpay)](https://youtu.be/Wy-zwXvscDI) | No new slice — the buying decision for the desk's platform layer (which package, the real monthly cost). Uses the chartbook ([`sierra/chartbooks/OPTD.Cht`](sierra/chartbooks/OPTD.Cht)) and the ORB study it unlocks ([`sierra/studies/OPTD_Opening_Range.cpp`](sierra/studies/OPTD_Opening_Range.cpp)) |
